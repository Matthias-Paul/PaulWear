  ...
  const handleCreateCheckout = async (e) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/pay/init`,
        {
          method: "POST",
        ...
        <div className="bg-white pt-5 rounded-lg md:p-25 lg:p-10 ">
          ...
          <form onSubmit={handleCreateCheckout}>
            ...
            <div className="mb-4">
              ...
              <input
                type="email"
                className="w-full mt-1 rounded p-2 border border-gray-400"
            ...
            <div className="grid grid-cols-2 gap-4 mb-4  ">
              <div className="  ">
                ...
                <input
                  type="text"
                  required
                  className="w-full p-2 rounded p-2 border border-gray-400 "
            ...
            <div className="mt-4  ">
              <button
                type="submit"
                className="w-full active:border active:border-gray-400 active:bg-white active:text-black py-2 font-medium text-lg cursor-pointer bg-black text-white rounded  "
              >
                {" "}
                Continue To Payment{" "}
              </button>
        ...
        <div className="bg-gray-50 py-4 px-2 sm:px-5 lg:px-13 mb-15 lg:mb-0 rounded-lg">
          ...
          <div className="border-t border-gray-700 py-4 mb-4 ">
            {myCart?.products?.map((product) => (
              <div
                key={product?._id}
                className="py-2 text-sm  border-b border-gray-700 flex items-start justify-between "
              >
                <div className="flex text-sm sm:text-lg  items-start justify-center ">
                  <img
                    src={product?.image}
                    alt={product?.name}
                    className="w-20 h-24 sm:w-26 sm:h-30 object-cover rounded-sm flex-shrink-0 mr-4"
                  />
                  <div>
