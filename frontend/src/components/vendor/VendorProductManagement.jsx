...
const VendorProductManagement = () => {
  ...
  const fetchVendorProducts = async ({ pageParam = 1 }) => {
    const res = await fetch(
      ...
      }/api/product/vendor?${searchParams.toString()}&page=${pageParam}&limit=15`,
      {
        method: "GET",
        credentials: "include",
      }
    ...
    <>
      <div className="pt-[82px] md:pt-5 pb-20 ">
        ...
        <Link to="/vendor/products/add">
          <button className="py-1 px-2  lg:px-4 lg:text-xl mb-2 rounded bg-gray-900 hover:bg-gray-700 text-white cursor-pointer  ">
            {" "}
            Add Product{" "}
          </button>
        </Link>
        ...
        <div className="text-xs sm:text-sm text-gray-500 px-3 mb-3 italic">
          <strong className="font-semibold">Notice:</strong> Please delete
          products that are no longer in stock or unavailable. This helps
          maintain an accurate store and a better shopping experience for
          buyers.
        </div>
