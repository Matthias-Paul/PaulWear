...
const Payout = () => {
  ...
  const fetchListOfBanks = async () => {
    const res = await fetch(
      `${import.meta.env.VITE_BACKEND_URL}/api/listOfBanks`,
      {
        method: "GET",
        credentials: "include",
      }
  ...
  const fetchAccountName = async () => {
    const res = await fetch(
      ...
      }/api/vendor/resolveName?accountNumber=${accountNumber}&bankCode=${bankCode}`,
      {
        method: "GET",
        credentials: "include",
      }
  ...
  const fetchBankDetails = async () => {
    const res = await fetch(
      `${import.meta.env.VITE_BACKEND_URL}/api/vendor/account`,
      {
        method: "GET",
        credentials: "include",
      }
  ...
  const updateAccountMutation = useMutation({
    mutationFn: async () => {
      // Send user details to the backend
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/vendor/account/update`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
  ...
  const fetchPaymentHistory = async ({ pageParam = 1 }) => {
    const res = await fetch(
      `${
        import.meta.env.VITE_BACKEND_URL
      }/api/vendor/payout/history?page=${pageParam}&limit=15`,
      {
        method: "GET",
        credentials: "include",
      }
  ...
  console.log("payoutHistoryData", payoutHistoryData)
  const vendorPayoutHistory =
    payoutHistoryData?.pages.flatMap((page) => page.vendorPayoutHistory) || [];
  console.log(vendorPayoutHistory);

  const stats = [
    {
      title: "Total Amount Of Payout",
      value: bankDetails?.account?.totalBalance
    ...
    },
    {
      ...
      value: bankDetails?.account?.pendingBalance
    ...
    },
    {
      ...
      value: bankDetails?.totalPayoutHistory?.toLocaleString() || "--",
      icon: <FaReceipt className="text-green-500" size={18} />,
      bg: "bg-gray-100",
    },
  ];
  ...
  return (
    <div className="w-full pb-20 pt-[90px] md:pt-4 mx-auto pr-[12px] md:pr-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {stats?.map((stat, index) => (
          ...
          >
            ...
            <p className="text-xl md:text-lg lg:text-2xl font-bold text-gray-800 mt-2 truncate">
              {stat?.value}
            </p>
          </div>
        ))}
      ...
      {bankDetails?.account || bankDetailsIsLoading ? (
        <>
          {bankDetails && <h5 className="mt-10 text-lg  "> Payout Account </h5>}
            ...
            <div className="border flex gap-x-2 justify-between items-start border-gray-400 mt-2 p-4 sm:p-6 rounded-lg shadow-md">
              <div>
                <h2 className="text-xl font-semibold mb-2">
                  {bankDetails?.account?.accountName}
                </h2>
                <h2 className="text-md mb-1">
                  {bankDetails?.account?.bankAccountNumber}
                </h2>
              ...
              <div className="  border flex flex-col gap-x-2 justify-between items-start border-gray-400 mt-2 p-4 sm:p-6 rounded-lg shadow-md">
                <div className=" flex justify-between items-center w-full gap-x-2 ">
                  ...
                  <button
                    ...
                    className="px-2 p-1 rounded bg-red-600 hover:bg-red-500 cursor-pointer text-white   "
                  >
                    Cancel
                  </button>
                </div>
                <form onSubmit={handleUpdate}>
                  ...
                  <div className="mb-6">
                    ...
                    >
                      ...
                      {banks.map((bank) => (
                        <option
                          key={`${bank?.code}-${bank?.name}-${bank?.slug}`}
                  ...
                  {userBankName && !nameError && (
                    <div className="mb-4">
                      <p className="text-md bg-green-100 p-2 rounded border border-green-400">
                        {userBankName}
                      </p>
                    </div>
                  )}
                  {nameError && (
                    <div className="mb-4">
                      <p className="text-md bg-red-100 p-2 rounded border border-red-400 text-red-600">
                        {nameError}
                      </p>
                    </div>
                  )}
