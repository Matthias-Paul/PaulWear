...
export const getOrders = async (req, res) => {
  try {
    ...
    const orders = await Order.find({ user: req?.user?._id })
      .populate("vendor", "email storeName contactNumber")
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
...
export const getVendorOrders = async (req, res) => {
  try {
    ...
    const vendorOrders = await Order.find(filter)
  ...
  try {
    ...
    if (order.isReceived) {
      return res
        ...
        .json({
          success: false,
          message: "You cannot cancel an order after marking it as received.",
        });
    }
    ...
    if (order.cancelRequested) {
      return res
        ...
        .json({
          success: false,
          message:
            "You have already requested for cancellation please be patient while the team review your submission.",
        });
    }
    ...
    await ActivityLog.create({
      ...
      actionType: "order_cancellation_requested",
      description: `User requested cancellation for order #${order._id}`,
      status: "success",
    });
    ...
    <div style="max-width: 600px; margin: auto; background-color: #ffffff; border-radius: 10px; overflow: hidden; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.06);">
      ...
      <div style="padding: 30px;">
        ...
        <p style="font-size: 15px; color: #4b5563; line-height: 1.6;">
          A buyer, <strong>${
            order.buyerName
          }</strong>, has submitted a request to cancel their order.
        </p>
...
`,
    ...
    return res.status(200).json({
      success: true,
      message:
        "Your cancellation request has been received. Our team will review it and get back to you shortly via email.",
    });
  } catch (err) {
    console.error(err);
    return res
